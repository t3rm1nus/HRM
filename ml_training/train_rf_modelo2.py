#!/usr/bin/env python3
"""
Entrena un modelo Random Forest para múltiples activos (BTC, ETH).
Usa datasets generados y los combina en un modelo único multiasset.

Salida:
- models/modelo1_rf_multiasset.pkl (modelo entrenado)
- models/modelo1_rf_multiasset.meta.json (metadatos con métricas por símbolo)
"""
...
def main():
    ...
    # Random Forest Classifier con parámetros optimizados
    rf = RandomForestClassifier(
        n_estimators=500,           # Más árboles para mejor generalización
        max_depth=None,             # Sin límite de profundidad (controlado por min_samples_split)
        min_samples_split=4,        # Mínimo para dividir nodos
        min_samples_leaf=2,         # Mínimo en hojas
        max_features='sqrt',        # Features por división (sqrt es bueno para clasificación)
    ...
    print("🎯 Entrenando Random Forest multiasset...")
    rf.fit(X_train, y_train)
    
    # Out-of-bag score (si está disponible)
    ...
    for i, (_, row) in enumerate(feature_importance.head(10).iterrows()):
        bar = "█" * int(row['importance'] * 50)  # Barra visual
    ...
    print(f"   🌳 Árboles: {rf.n_estimators}, Profundidad: {rf.max_depth or 'Sin límite'}")
