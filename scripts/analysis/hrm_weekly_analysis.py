...
class HRMWeeklyAnalyzer:
    ...
    def analyze_all(self):
        ...
        self._report_hacienda_analysis()
        self._validate_tax_data_consistency()
    ...
    def _load_hacienda_data(self):
        """Load tax/fiscal data from hacienda module"""
        print("📁 Loading hacienda (tax) data...")
        
        hacienda_dir = self.base_path / "hacienda"
        
        # Load operations
        operations_file = hacienda_dir / "operaciones.csv"
        if operations_file.exists():
            try:
                ...
                df = df[df['timestamp'] >= self.cutoff_date]
                self.tax_operations = df.to_dict('records')
                print(f"   ✅ Loaded {len(self.tax_operations)} tax operations")
            except Exception as e:
                print(f"   ⚠️ Error reading tax operations: {e}")
        else:
            print("   ⚠️ No tax operations file found")
        
        # Load FIFO positions
        positions_file = hacienda_dir / "posiciones_fifo.json"
        if positions_file.exists():
            try:
                with open(positions_file, 'r') as f:
                    self.tax_positions = json.load(f)
                total_positions = sum(len(positions) for positions in self.tax_positions.values())
                print(f"   ✅ Loaded {total_positions} FIFO positions")
            except Exception as e:
                print(f"   ⚠️ Error reading FIFO positions: {e}")
        else:
            print("   ⚠️ No FIFO positions file found")
        
        # Load realized gains
        gains_file = hacienda_dir / "ganancias_realizadas.csv"
        if gains_file.exists():
            try:
                ...
                df = df[df['sell_date'] >= self.cutoff_date]
                self.tax_gains = df.to_dict('records')
                total_gains = sum(g.get('gain_loss', 0) for g in self.tax_gains)
                print(f"   ✅ Loaded {len(self.tax_gains)} realized gains (total: ${total_gains:.2f})")
            except Exception as e:
                print(f"   ⚠️ Error reading realized gains: {e}")
        else:
            print("   ⚠️ No realized gains file found")
    ...
    def _report_hacienda_analysis(self):
        """Report fiscal/tax analysis from hacienda data"""
        print("\n" + "=" * 80)
        print("🏛️ HACIENDA (TAX) ANALYSIS")
        print("=" * 80)
        
        if not self.tax_operations and not self.tax_gains:
            print("\n   ⚠️ No tax data available")
            print("      Run trading to generate fiscal records")
            return
        
        # Tax operations summary
        if self.tax_operations:
            df_ops = pd.DataFrame(self.tax_operations)
            total_tax_ops = len(df_ops)
            buy_ops = len(df_ops[df_ops['side'].str.lower() == 'buy']) if 'side' in df_ops.columns else 0
            sell_ops = len(df_ops[df_ops['side'].str.lower() == 'sell']) if 'side' in df_ops.columns else 0
            
            print(f"\n📊 TAX OPERATIONS SUMMARY:")
            print(f"   Total Operations: {total_tax_ops}")
            print(f"   Buy Operations: {buy_ops}")
            print(f"   Sell Operations: {sell_ops}")
            
            if 'total_value' in df_ops.columns:
                total_buy_value = df_ops[df_ops['side'].str.lower() == 'buy']['total_value'].sum() if buy_ops > 0 else 0
                total_sell_value = df_ops[df_ops['side'].str.lower() == 'sell']['total_value'].sum() if sell_ops > 0 else 0
                print(f"   Total Buy Value: ${total_buy_value:,.2f}")
                print(f"   Total Sell Value: ${total_sell_value:,.2f}")
            
            if 'commission' in df_ops.columns:
                total_commissions = df_ops['commission'].sum()
                print(f"   Total Commissions: ${total_commissions:,.2f}")
            
            # By symbol
            if 'symbol' in df_ops.columns:
                print(f"\n📈 OPERATIONS BY SYMBOL:")
                for symbol in df_ops['symbol'].unique():
                    symbol_ops = df_ops[df_ops['symbol'] == symbol]
                    print(f"   {symbol}: {len(symbol_ops)} ops")
        
        # Realized gains/losses
        if self.tax_gains:
            df_gains = pd.DataFrame(self.tax_gains)
            total_gains = df_gains[df_gains['gain_loss'] > 0]['gain_loss'].sum() if 'gain_loss' in df_gains.columns else 0
            total_losses = abs(df_gains[df_gains['gain_loss'] < 0]['gain_loss'].sum()) if 'gain_loss' in df_gains.columns else 0
            net_gains = total_gains - total_losses
            
            print(f"\n💰 REALIZED GAINS/LOSSES:")
            print(f"   Total Gains: ${total_gains:,.2f}")
            print(f"   Total Losses: ${total_losses:,.2f}")
            print(f"   Net Realized: ${net_gains:,.2f}")
            
            if net_gains > 0:
                print(f"      ✅ Net positive for tax purposes")
            else:
                print(f"      ℹ️ Net loss (can offset other gains)")
            
            # By symbol
            if 'symbol' in df_gains.columns and 'gain_loss' in df_gains.columns:
                print(f"\n📈 GAINS/LOSSES BY SYMBOL:")
                for symbol in df_gains['symbol'].unique():
                    symbol_gains = df_gains[df_gains['symbol'] == symbol]['gain_loss'].sum()
                    print(f"   {symbol}: ${symbol_gains:,.2f}")
        
        # FIFO positions summary
        if self.tax_positions:
            print(f"\n📦 FIFO POSITIONS:")
            for symbol, positions in self.tax_positions.items():
                total_qty = sum(pos.get('remaining_quantity', 0) for pos in positions)
                total_cost = sum(pos.get('remaining_quantity', 0) * pos.get('price', 0) for pos in positions)
                print(f"   {symbol}: {total_qty:.6f} units, Cost basis: ${total_cost:,.2f}")

    def _validate_tax_data_consistency(self):
        """Validate consistency between operational and fiscal data"""
        print("\n" + "=" * 80)
        print("🔍 TAX DATA CONSISTENCY VALIDATION")
        print("=" * 80)
        
        if not self.trades and not self.tax_operations:
            print("\n   ℹ️ No data to validate")
            return
        
        if not self.tax_operations:
            print("\n   ⚠️ No tax operations found but trades exist")
            print("      Tax tracking may not be enabled")
            return
        
        # Compare trade counts
        trade_count = len(self.trades)
        tax_op_count = len(self.tax_operations)
        
        print(f"\n📊 DATA COMPARISON:")
        print(f"   Operational Trades: {trade_count}")
        print(f"   Tax Operations: {tax_op_count}")
        
        if trade_count == tax_op_count:
            print(f"      ✅ Trade counts match")
        else:
            diff = abs(trade_count - tax_op_count)
            print(f"      ⚠️ Mismatch: {diff} operations differ")
            if tax_op_count > trade_count:
                print(f"         Tax system has more records (may include manual entries)")
            else:
                print(f"         Some trades not recorded for tax (check tax_tracker)")
        
        # Compare by side (Buy/Sell)
        if self.trades and self.tax_operations:
            df_trades = pd.DataFrame(self.trades)
            df_tax = pd.DataFrame(self.tax_operations)
            
            if 'side' in df_trades.columns and 'side' in df_tax.columns:
                trade_buys = len(df_trades[df_trades['side'].str.lower() == 'buy'])
                trade_sells = len(df_trades[df_trades['side'].str.lower() == 'sell'])
                tax_buys = len(df_tax[df_tax['side'].str.lower() == 'buy'])
                tax_sells = len(df_tax[df_tax['side'].str.lower() == 'sell'])
                
                print(f"\n📊 SIDE COMPARISON:")
                print(f"   Operational - Buy: {trade_buys}, Sell: {trade_sells}")
                print(f"   Tax System  - Buy: {tax_buys}, Sell: {tax_sells}")
                
                if trade_buys == tax_buys and trade_sells == tax_sells:
                    print(f"      ✅ Buy/Sell counts match")
                else:
                    if trade_buys != tax_buys:
                        print(f"      ⚠️ Buy count mismatch: {trade_buys} vs {tax_buys}")
                    if trade_sells != tax_sells:
                        print(f"      ⚠️ Sell count mismatch: {trade_sells} vs {tax_sells}")
        
        # Check for orphaned tax records (no matching trade)
        print(f"\n🔍 VALIDATION CHECKS:")
        
        # Check PnL consistency
        if self.trades and 'realized_pnl' in pd.DataFrame(self.trades).columns:
            trade_pnl = pd.DataFrame(self.trades)['realized_pnl'].sum()
            tax_pnl = sum(g.get('gain_loss', 0) for g in self.tax_gains) if self.tax_gains else 0
            
            print(f"   Operational PnL: ${trade_pnl:,.2f}")
            print(f"   Tax Realized Gains: ${tax_pnl:,.2f}")
            
            # Allow small differences due to timing/FIFO calculation
            if abs(trade_pnl - tax_pnl) < 1.0:
                print(f"      ✅ PnL values match (within $1 tolerance)")
            else:
                diff_pct = abs(trade_pnl - tax_pnl) / max(abs(trade_pnl), 1) * 100
                if diff_pct < 5:
                    print(f"      ⚠️ PnL differs by ${abs(trade_pnl - tax_pnl):,.2f} ({diff_pct:.1f}%) - minor variance")
                else:
                    print(f"      🔴 Significant PnL difference: ${abs(trade_pnl - tax_pnl):,.2f} ({diff_pct:.1f}%)")
                    print(f"         Review FIFO calculations and trade matching")
        
        # Final validation status
        print(f"\n📋 VALIDATION SUMMARY:")
        all_ok = (trade_count == tax_op_count)
        
        if all_ok:
            print(f"   ✅ Tax data is consistent with operational data")
            print(f"   ✅ Fiscal records ready for tax declaration")
        else:
            print(f"   ⚠️ Some inconsistencies detected between operational and tax data")
            print(f"   ℹ️ Review recommended before tax filing")
