...
def gestionar_exposicion(universo, portfolio_state, market_data, regime):
    ...
    try:
        ...
        btc_close = market_data.get("BTCUSDT", {}).get("close", 50000.0)
        if isinstance(btc_close, list):
            btc_price = btc_close[-1] if btc_close else 50000.0
        else:
            btc_price = float(btc_close)

        eth_close = market_data.get("ETHUSDT", {}).get("close", 4327.46)
        if isinstance(eth_close, list):
            eth_price = eth_close[-1] if eth_close else 4327.46
        else:
