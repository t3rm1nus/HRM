...
## 🧹 **SISTEMA DE LIMPIEZA AUTOMÁTICA (2025)**

**NUEVA FUNCIONALIDAD 2025:** HRM incluye un **sistema de limpieza automática completo** que se ejecuta automáticamente al inicio de cada sesión de trading.

### 🎯 **Características del Sistema de Limpieza**

#### ✅ **Limpieza Automática al Inicio**
Cada vez que ejecutas `main.py`, el sistema automáticamente:
- ✅ **Elimina logs antiguos** (`logs/`, `test_logs/`, `backtesting/logs/`)
- ✅ **Limpia datos temporales** (`__pycache__/`, `*_cache.json`, archivos temporales)
- ✅ **Resetea estado de portfolio** (`portfolio_state_live.json`, `hacienda/posiciones_fifo.json`)
- ✅ **Limpia datos de inferencia** (cache BERT, sentiment analysis)
- ✅ **Recrea directorios frescos** (`logs/`, `data/datos_inferencia/`, `results/`)

#### 📋 **Archivos Elinados por el Sistema**
```text
...
└── **/__pycache__/               # Cache de Python (todos los directorios)
...
├── sentiment_cache_*.json          # Cache de sentiment analysis
├── sentiment_inference_*.csv       # Datos de inferencia de sentimiento
└── kk.py                          # Archivo de debug temporal
```

#### 🚀 **Ejecución Automática**

**Cada vez que inicias HRM:**
```bash
python main.py
...
#### 🔧 **Ejecución Manual**
También puedes ejecutar la limpieza directamente:
```bash
# Limpieza completa
...
# Limpiar con directorio específico
python system_cleanup.py --path /ruta/a/directorio
```

#### 🛡️ **Características de Seguridad**
- ✅ **No elimina modelos entrenados** (`.pkl`, `.h5` archivos)
- ✅ **No toca directorios de Git** (`.git/` queda intacto)
- ✅ **Manejo seguro de errores** (continúa aunque algunos archivos fallen)
...
```
HRM/
├── 📄 system_cleanup.py           # Script principal de limpieza
├── 📄 README_SYSTEM_CLEANUP.md    # Documentación detallada
└── 📄 main.py                     # Integración automática
    └── from system_cleanup import SystemCleanup  # Import
    └── cleanup.perform_full_cleanup()           # Ejecución
```

### 🎯 **Beneficios del Sistema**

#### 🚀 **Consistencia**
- **Estado limpio garantizado** al iniciar cada sesión
- **No acumulación de datos basura** entre pruebas
- **Entorno consistente** para desarrollo y producción

#### ⚡ **Eficiencia**
- **Ejecución automática** sin intervención manual
- **Limpieza inteligente** que preserva datos importantes
- **Rápido y confiable** (< 2 segundos en sistemas típicos)

#### 🛡️ **Seguridad**
- **No elimina modelos** o configuraciones críticas
- **Manejo seguro** de errores y permisos
- **Logs detallados** para auditoría de limpieza
...
🧹 Running system cleanup before startup...
🧹 Cleaning logs directories...
🗂️ Directorio eliminado: logs: [Ruta completa eliminada]
🧹 Cleaning hcacienda files (tax system)...
🗑️ Eliminado: hacienda: hacienda/posiciones_fifo.json
🧹 Cleaning portfolio data...
🗑️ Eliminado: portfolio: portfolio_*.json pattern matches
🧹 Cleaning inference and sentiment data...
🗂️ Directorio eliminado: inference data: data/datos_inferencia
📁 Creating fresh directories...
✅ Directorio creado: logs/
✅ Directorio creado: data/datos_inferencia/
✅ Directorio creado: results/
✅ Cleanup completed successfully - 45 files, 12 directories removed
...
```python
# En main.py - Ejecución automática
async def main():
    # 🧹 SYSTEM CLEANUP - Run before starting HRM system
    cleanup = SystemCleanup()  # Instancia la clase
    cleanup_result = cleanup.perform_full_cleanup()  # Ejecuta limpieza completa

    if not cleanup_result.get("success", False):
        logger.warning("⚠️ Limpieza completada con warnings")
    else:
        logger.info(f"✅ Limpieza exitosa - {cleanup_result['deleted_files']} archivos, {cleanup_result['deleted_dirs']} directorios eliminados")

    # Continúa con inicialización normal...
```

### 🎉 **Resultado Final**

**Sistema HRM con limpieza automática:**
- ✅ **Se limpia solo** cada vez que inicia
- ✅ **Estado fresco garantizado** para cada sesión
- ✅ **Preserva datos importantes** (modelos, configuraciones)
- ✅ **Funciona sin intervención** del usuario
- ✅ **Logs detallados** de todas las operaciones

**¡HRM ahora tiene un sistema de limpieza automática completo y seguro!** 🧹⚡✨
